...
import ufl
from ufl import grad, curl, Measure
...
nu_value = fem.Constant(domain, default_scalar_type(nu_val))
...
a00 = ufl.inner(nu * curl(u), curl(v)) * dx
a01 = ufl.inner(sigma * grad(u1), v) * dx
a11 = ufl.inner(sigma * ufl.grad(u1), ufl.grad(v1)) * dx

zero_vec = fem.Constant(domain, PETSc.ScalarType((0.0, 0.0, 0.0)))
L0 = ufl.inner(zero_vec, v) * dx
L1 = fem.Constant(domain, PETSc.ScalarType(0.0)) * v1 * dx
...
A_file = VTXWriter(domain.comm, "A_field_steady.bp", A_vis, "BP4")
A_vis.interpolate(u_n)
A_file.write(t)

B = curl(u_n)
B_vis = Function(vector_vis)
B_file = VTXWriter(domain.comm, "B_field_steady.bp", B_vis, "BP4")
...
E_file = VTXWriter(domain.comm, "E_field_steady.bp", E_vis, "BP4")
E_file.write(t)
...
J_file = VTXWriter(domain.comm, "J_field_steady.bp", J_vis, "BP4")
J_file.write(t)

u_n1_file = VTXWriter(domain.comm, "u_n1_field_steady.bp", u_n1, "BP4")
u_n1_file.write(t)


par_print(comm, f"L2 norm of u_n field {L2_norm(u_n)}")
...
par_print(comm, f"J norm is {L2_norm(J_ind)}")
