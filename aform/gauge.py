...
import numpy as np
from ufl import SpatialCoordinate, as_vector, pi, curl, cos, TrialFunction, TestFunction, inner, dx
...
beta = Function(DG)
beta.interpolate(lambda x: np.where(x[0] <= 0.5, 1.0, 0.0))
...
from ufl import div
const = fem.Constant(domain, PETSc.ScalarType(1e3))

a = form(inner(alpha * curl(A), curl(v)) * dx + inner(beta * A, v) * dx + const * (inner(div(A), div(v)) * dx))
L = form(inner(f,v) * dx)
