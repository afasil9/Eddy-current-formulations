...
def error_Hcurl(uh, u_ex, degree_raise=4):
    # Create higher order function space
    degree = uh.function_space.ufl_element().degree
    family = uh.function_space.ufl_element().family_name
    mesh = uh.function_space.mesh
    ...
    # Integrate both L2 and curl errors
    error_L2 = form(inner(e_W, e_W) * dx)
    error_curl = form(inner(curl(e_W), curl(e_W)) * dx)
    
    # Assemble local errors
    error_L2_local = assemble_scalar(error_L2)
    error_curl_local = assemble_scalar(error_curl)
    
    # Reduce across processes
    error_L2_global = mesh.comm.allreduce(error_L2_local, op=MPI.SUM)
    error_curl_global = mesh.comm.allreduce(error_curl_local, op=MPI.SUM)
    
    # Return H(curl) norm
    return np.sqrt(error_L2_global + error_curl_global)


def save_function(v, filename, t, B_field=False):
    ...
    if B_field:
        ...
        w = Function(W)
        B = curl(v)
        Bexpr = Expression(B, w.function_space.element.interpolation_points())
        w.interpolate(Bexpr)
        w.name = v.name
        with VTXWriter(mesh.comm, filename, w, "BP4") as file:
...
def convert_facet_tags(submsh, cell_emap, facet_tags):
    ...
    # cells may be -1.
    cells[cells >= 0] = cell_emap.sub_topology_to_topology(
        cells[cells >= 0], inverse=True
    )
...
def create_mesh_fenics(comm, n, boundaries):
    ...
    ct = mesh.meshtags(
        msh,
        tdim,
        np.array(range(msh.topology.index_map(tdim).size_local), dtype=np.int32),
...
def refine_mesh(domain, ft, ct, refinement_level):
    for i in range(refinement_level):
        ...
        fine_mesh, parent_cell, parent_facet = refine(
            domain, option=RefinementOption.parent_cell_and_facet
        )
...
def boundary_marker_copper(x):
    ...
    # X-normal boundaries
    x_bdy = np.logical_or(
        np.isclose(x[0], 0.0, atol=tol), np.isclose(x[0], 1000.0, atol=tol)
    )
    markers[x_bdy] = 1

    # Y-normal boundaries
    y_bdy = np.logical_or(
        np.isclose(x[1], 0.0, atol=tol), np.isclose(x[1], 1000.0, atol=tol)
    )
    markers[y_bdy] = 2

    # Z-normal boundaries (note: interval [425, 575])
    z_bdy = np.logical_or(x[2] <= 425.0 + tol, x[2] >= 575.0 - tol)
    markers[z_bdy] = 3
